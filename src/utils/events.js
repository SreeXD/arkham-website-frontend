...
export const homeScroll = (e) => {
    for (let i = 0; i < 3; ++i) {
        ...
        if (sbbView >= 0.5 && sbbView <= (window.innerHeight <= 700 ? 1.75 : 1.5) && !vis[i]) {
            if (last[i]) last[i].kill(); 

            last[i] = gsap.to(sectionVideo, { duration: 0.3, opacity: 1 });
        ...
        if (vis[i] && (sbbView < 0.5 || sbbView > (window.innerHeight <= 700 ? 1.75 : 1.5))) {
            if (last[i]) last[i].kill(); 
